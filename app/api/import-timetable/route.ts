...
async function openAiParseTimetableImage(
  ...
  mimeType: string,
  imageBase64: string,
): Promise<{ ok: true; payload: ReturnType<typeof parseTimetableImportPayload> } | { ok: false; status: number }> {
  const maxAttempts = 2;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
          ...
          messages: [
            { role: "system", content: SYSTEM_PROMPT },
            {
              ...
              content: [
                {
                  type: "text",
                  text: "Extract all courses and weekly class slots from this timetable image.",
                },
                {
                  type: "image_url",
                  image_url: {
      ...
      const payload = parseTimetableImportPayload(parsed);
      if (!payload) {
        if (attempt < maxAttempts) {
          continue;
        }
...
export async function POST(request: Request) {
  ...
  let body: ImportTimetableBody;
  try {
    body = (await request.json()) as ImportTimetableBody;
  } catch {
  ...
  const result = await openAiParseTimetableImage(apiKey, model, mimeType, imageBase64);
  if (!result.ok) {
    if (result.status === 422) {
      return NextResponse.json(
        { error: "Could not read a timetable from that photo. Try a clearer, well-lit picture." },
        { status: 422 },
      );
    }
    return NextResponse.json({ error: "Could not analyse the timetable photo. Try again." }, { status: 502 });
  }
